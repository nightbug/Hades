...
import Hades.cards.bas.Defend;
import Hades.cards.bas.Strike;
import Hades.util.BetterSpriterAnimation;
...
import static Hades.Hades.*;
import static Hades.util.actionShortcuts.p;

public class chr_hades extends CustomPlayer {

    public static class Enums {
        @SpireEnum
        public static PlayerClass HADES;
        @SpireEnum(name = "HADES_COLOUR")
        public static AbstractCard.CardColor HADES_COLOUR;
        @SpireEnum(name = "HADES_COLOUR")
    ...
    private static final String ID = makeID(chr_hades.class.getSimpleName());
    public static final CharacterStrings characterStrings = CardCrawlGame.languagePack.getCharacterString(ID);
    private static final String[] NAMES = characterStrings.NAMES;
    private static final String[] TEXT = characterStrings.TEXT;


    public chr_hades(String name, PlayerClass setClass) {
        super(name, setClass, new CustomEnergyOrb(orbTextures, getModID() + "Resources/images/char/mainChar/orb/vfx.png", null), new BetterSpriterAnimation(
                getModID() + "Resources/images/char/mainChar/HadesAnimation.scml"));
    ...
    public AbstractPlayer newInstance() {
        return new chr_hades(name, chosenClass);
    }
    ...
    public void damage(DamageInfo info) {
        ...
        if (info.owner != null && info.type != DamageInfo.DamageType.THORNS && trueDamage > 0) {
            if (this.isDead) { playDeathAnimation(); }// add death anim later
            else {
                // hit
            }
        }
    ...
    public static void runAnimation(String anim) {
        if (p() instanceof chr_hades) { ((chr_hades) p()).runAnim(anim); }
    }
